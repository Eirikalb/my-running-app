// Shared BPM DSP. Used by both the in-browser detector (lib/bpm-detect.js) and
// the Node crawler (lib/bpm-node.mjs) so they produce identical numbers.
//
// Approach: isolate the percussive low end with a band filter, find rhythmic
// peaks above an adaptive threshold, histogram the intervals between nearby
// peaks, and take the most common tempo. The only environment-specific bit is
// the OfflineAudioContext constructor, which the caller passes in.

export async function bpmFromAudioBuffer(audioBuffer, OfflineCtor) {
